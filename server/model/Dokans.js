...
module.exports.addDokan = async function (id, name, email) {
	try {
		const sellerId = email + "seller";
		const newDokan = await this.createDokan(name);
		console.log(newDokan.name);
		if (newDokan) {
			try {
...
module.exports.getDokan = async (id) => {
	...
	try {
		if (key) {
			const dokan = await Dokan.findById(key);
